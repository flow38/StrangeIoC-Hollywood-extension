...
namespace falcy.strange.extensions.hollywood.impl
{
    public class DirectorBinder : Binder, IMediationBinder
    {
        ...
        protected virtual void injectActorAndChildren(IHollywoodView actor)
        {
            ...
            var actors = mono.GetComponentsInChildren(typeof (IActor), true);

            var aa = actors.Length;
            for (var a = aa - 1; a > -1; a--)
            {
                var anActor = actors[a] as IActor;
                if (anActor != null)
                {
        ...
        protected virtual void mapView(IHollywoodView hollyView, IMediationBinding binding)
        {
            ...
            if (bindings.ContainsKey(viewType))
            {
                ...
                for (var a = 0; a < aa; a++)
                {
                    ...
                    injectionBinder.Bind<Director>().To(directorType);
                    IDirector director = injectionBinder.GetInstance<Director>();
                    injectionBinder.Unbind<Director>();
